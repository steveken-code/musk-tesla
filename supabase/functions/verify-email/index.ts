...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const ipLimit = checkRateLimit(`verify-email:ip:${clientIP}`, RATE_LIMIT_IP_MAX, RATE_LIMIT_IP_WINDOW);
    
    if (!ipLimit.allowed) {
      console.log(`Rate limit exceeded for IP: ${clientIP} on verify-email`);
